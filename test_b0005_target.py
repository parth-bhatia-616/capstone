...
def run_target_b0005_test():
    print("Re-configuring validation matrix: Setting Target Cell -> B0005...")
    
 
    soh_df = pd.read_parquet(SOH_DATA_PATH)
    features_soh = ['cycle', 'max_temp', 'min_voltage', 'internal_resistance', 'discharge_duration']
    
    # Invert the split: Train on B0006/B0007, Test on B0005
    train_soh = soh_df[soh_df['battery_id'].isin(['B0006', 'B0007'])]
    test_soh = soh_df[soh_df['battery_id'] == 'B0005']
    ...
    print(" SoH Shift Performance (Target: B0005) ---")
    ...
    train_df = df_soc[df_soc['battery_id'].isin(['B0006', 'B0007'])].copy()
    test_df = df_soc[df_soc['battery_id'] == 'B0005'].copy()
    ...
    # Capture telemetry window arrays exclusively from B0005 Cycle 50 for trace generation
    c50_df = test_df[test_df['cycle'] == 50]
    ...
    # Load state configuration parameters
    ...
    # Slicing vectors to plot cycle 50 specifically
    ...
    print("SoC Shift Performance (Target: B0005) ---")
    ...
    plt.plot(plot_times, y_true_50, label='True SoC (B0005 Reference)', color='white', linewidth=2.5)
    plt.plot(plot_times, preds_50, label='Conv-BiLSTM Cross-Prediction', color='#ff355e', linestyle='--', linewidth=2)
    
    plt.title('Cross-Battery SoC Evaluation Vector — Target: Battery B0005 (Cycle 50)', color='white', fontsize=12, pad=12)
    ...
    save_path = PROCESSED_DIR / "b0005_cross_validation.png"
    plt.savefig(save_path, dpi=300, bbox_inches='tight', facecolor='black')
    print(f"Target shifted alignment chart rendered cleanly to: {save_path}")

if __name__ == "__main__":
    run_target_b0005_test()